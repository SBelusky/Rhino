<template>
    <div id="form-view">
        <window-title :small-title="type == 'detail' ? '| detail kategórie' : '| editácia kategórie'" :big-title="data.name" />
...
export default {
    ...
    mounted() {
        axios
            .get("http://localhost:8080/api/project/" + this.actualProject + "/detail/category/" + this.$route.params.id, {
                headers: {
    ...
    methods: {
        submitForms() {
            ...
            axios
                .post("http://localhost:8080/api/project/" + this.actualProject + "/edit/category/" + this.$route.params.id, editData, {
                    headers: {
                        Authorization: "Bearer " + localStorage.getItem("token")
                    }
                })
